...
abstract class BaseDataManager<T> {
  ...
  Future<bool> addWithAuth(T item) async => await manager.addWithAuth(item);
  Future<T?> getById(String userId, String id) async =>
      await manager.getById(userId, id);
  Future<List<T>> getAll(String userId) async => await manager.getAll(userId);
  Future<List<T>> getAllWithAuth() async => await manager.getAllWithAuth();
  ...
  Future<List<T>> getLocalAll() async => await manager.getLocalAll();
  Future<T?> getLocalById(String id) async => await manager.getLocalById(id);
  Future<void> saveLocal(List<T> items) async => await manager.saveLocal(items);
  Future<void> addLocal(T item) async => await manager.addLocal(item);
  Future<void> updateLocal(T item) async => await manager.updateLocal(item);
  Future<void> clearLocal() async => await manager.clearLocal();

  Future<List<T>> sync(String userId) async => await manager.sync(userId);
  Future<List<T>> syncWithAuth() async => await manager.syncWithAuth();
...
abstract class BaseHiveDataManager<T> {
  ...
  ) async => await manager.updatePartial(userId, id, updates);
  Future<bool> updatePartialWithAuth(
    String id,
    Map<String, dynamic> updates,
  ) async => await manager.updatePartialWithAuth(id, updates);
  Future<List<T>> getAllWithQuery(
    String userId, {
    Map<String, dynamic>? whereConditions,
  }) async =>
      await manager.getAllWithQuery(userId, whereConditions: whereConditions);

  Future<List<T>> getLocalAll() async => await manager.getLocalAll();
  Future<T?> getLocalById(String id) async => await manager.getLocalById(id);
  Future<void> saveLocal(List<T> items) async => await manager.saveLocal(items);
  Future<void> clearLocal() async => await manager.clearLocal();
  Future<int> getLocalCount() async => await manager.getLocalCount();
  ...
  Future<void> clearQueue() async => await manager.clearQueue();

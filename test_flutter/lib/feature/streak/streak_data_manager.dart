...
class StreakDataManager {
  ...
  Future<bool> addStreakDataWithRetry(
    String userId,
  ...
  ///
  /// **戻り値**: 成功時true、失敗時false（キューに追加された場合もfalse）
  Future<bool> updateStreakDataWithRetry(
    String userId,
    StreakData streakData,
  ) async {
  ...
  ///
  /// **戻り値**: 成功時true、失敗時false（キューに追加された場合もfalse）
  Future<bool> saveStreakDataWithRetry(
    String userId,
    StreakData streakData,
  ) async {
  ...
  Future<Map<String, dynamic>> trackFinished() async {
    try {
      ...
      if (currentData == null) {
        ...
        debugPrint(
          '✅ [trackFinished] ローカル保存完了: currentStreak=${newData.currentStreak}',
        );
        ...
        return {'success': true, 'message': '1日連続記録中！', 'streak': 1};
      }

      // 3. 同じ日かチェック
      if (_isSameDay(currentData.lastTrackedDate, now)) {
        // 同じ日でもlastModifiedだけ更新
        final updatedData = currentData.copyWith(lastModified: now);
      ...
      await updateLocalStreakData(updatedData);
      debugPrint(
        '✅ [trackFinished] ローカル更新完了: currentStreak=${updatedData.currentStreak}',
      );
      ...
      return {'success': false, 'message': 'エラーが発生しました', 'streak': 0};
    }
  }

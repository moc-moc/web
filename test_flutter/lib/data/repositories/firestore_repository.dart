  ...
  Future<bool> update(String userId, T item) async {
    try {
      ...
      if (success) {
        ...
        await LogMk.logInfo('✅ アイテム更新成功（Firestore + ローカル）: $itemId');
      } else {
  ...
  Future<List<T>> sync(String userId) async {
    ...
    return await LockMk.withLock(_syncLock, () async {
      try {
        ...
        // 4. Firestoreから削除されたアイテムをローカルからも削除（全量同期のときのみ実行）
        List<Map<String, dynamic>> currentLocalDataList = localDataList;
        if (isFullSync) {

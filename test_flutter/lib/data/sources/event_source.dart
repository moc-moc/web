...
class EventMk {
  ...
  /// 
  /// **戻り値**: リスナーを削除するための関数
  static void Function() on(
    EventType type,
    void Function(EventData) listener,
  ) {
  ...
  static void offAll(EventType type) {
    if (_listeners.containsKey(type)) {
      ...
      LogMk.logDebug('全イベントリスナー削除: ${type.name} ($count件)', tag: 'EventMk.offAll');
    }
  }

  /// 全てのリスナーを削除
  static void clearAllListeners() {
    final count = _listeners.values.fold<int>(0, (sum, list) => sum + list.length);
    _listeners.clear();
  ...
  static int getListenerCount([EventType? type]) {
    if (type != null) {
      ...
      final specificCount = _listeners.values.fold<int>(0, (sum, list) => sum + list.length);

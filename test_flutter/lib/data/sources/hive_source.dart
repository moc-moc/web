...
class HiveMk {
  ...
  static Future<void> saveAllToHive(
    String boxName,
  ...
  static Future<List<Map<String, dynamic>>> getAllFromHive(
    String boxName,
  ) async {
    try {
      final box = await Hive.openBox(boxName);
  ...
  /// 指定されたキーの最終同期時刻を設定する
  /// 同期時刻は'{boxName}_last_sync'として保存される
  static Future<void> setLastSyncTimeToHive(
    String boxName,
    DateTime time,
  ) async {
    try {
  ...
  static Future<List<String>> getAllBoxNames() async {
    try {
      ...
      return boxNames
          .cast<String>()
          .where((name) => !name.startsWith('_hive_'))
          .toList();
    } catch (e) {
  ...
  ///
  /// **戻り値**: 変更されたアイテムのリスト
  static Future<List<Map<String, dynamic>>> getDirtyItems(
    String boxName,
  ) async {
    try {
      final items = await getAllFromHive(boxName);
  ...
  static Future<int> getStorageSize() async {
    try {
      ...
      debugPrint(
        '✅ Hiveストレージサイズ取得: $totalSizeバイト (${(totalSize / 1024).toStringAsFixed(2)}KB)',
      );
      return totalSize;
    } catch (e) {
  ...
  static Future<int> clearOldestItems(
    ...
    try {
      ...
      debugPrint(
        '✅ Hive最古アイテム削除完了: $removeCount件削除 (残り${remainingItems.length}件)',
      );

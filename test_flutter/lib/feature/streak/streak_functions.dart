...
class StreakDataNotifier extends _$StreakDataNotifier {
  ...
  StreakData build() {
    debugPrint(
      '🔍 [StreakDataNotifier.build] ★★★ Provider初期化実行（keepAlive: true）★★★',
    );
  ...
  void updateStreak(StreakData newData) {
    debugPrint(
      '🔍 [StreakDataNotifier.updateStreak] 更新: ${newData.currentStreak}日連続',
    );
    state = newData;
  }
...
Future<StreakData> loadStreakDataHelper(dynamic ref) async {
  ...
  if (streakData != null) {
    debugPrint(
      '🔍 [loadStreakDataHelper] Firestoreから取得: ${streakData.currentStreak}日連続',
    );
  } else {
    // Firestoreから取得できない場合はデフォルト値
    final defaultData = await manager.getStreakDataOrDefault();
    debugPrint(
      '🔍 [loadStreakDataHelper] デフォルト値を使用: ${defaultData.currentStreak}日連続',
    );
  ...
  debugPrint(
    '🔍 [loadStreakDataHelper] Provider更新後の状態: ${updatedState.currentStreak}日連続',
  );
...
Future<StreakData> syncStreakDataHelper(dynamic ref) async {
  ...
  final updatedState = ref.read(streakDataProvider);
  debugPrint(
    '🔍 [syncStreakDataHelper] Provider更新後の状態: ${updatedState.currentStreak}日連続',
  );

...
class TotalDataNotifier extends _$TotalDataNotifier {
  ...
  TotalData build() {
    debugPrint(
      '🔍 [TotalDataNotifier.build] ★★★ Provider初期化実行（keepAlive: true）★★★',
    );
  ...
  void updateTotal(TotalData newData) {
    debugPrint(
      '🔍 [TotalDataNotifier.updateTotal] 更新: ${newData.totalLoginDays}日、${newData.totalWorkTimeMinutes}分',
    );
    state = newData;
  }
...
Future<TotalData> loadTotalDataHelper(dynamic ref) async {
  ...
  if (totalData != null) {
    debugPrint(
      '🔍 [loadTotalDataHelper] Firestoreから取得: ${totalData.totalLoginDays}日、${totalData.totalWorkTimeMinutes}分',
    );
  } else {
    // Firestoreから取得できない場合はデフォルト値
    final defaultData = await manager.getTotalDataOrDefault();
    debugPrint(
      '🔍 [loadTotalDataHelper] デフォルト値を使用: ${defaultData.totalLoginDays}日、${defaultData.totalWorkTimeMinutes}分',
    );
  ...
  debugPrint(
    '🔍 [loadTotalDataHelper] Provider更新後の状態: ${updatedState.totalLoginDays}日、${updatedState.totalWorkTimeMinutes}分',
  );
...
Future<TotalData> syncTotalDataHelper(dynamic ref) async {
  ...
  debugPrint(
    '🔍 [syncTotalDataHelper] 最終データ: ${totalData.totalLoginDays}日、${totalData.totalWorkTimeMinutes}分',
  );
  ...
  final updatedState = ref.read(totalDataProvider);
  debugPrint(
    '🔍 [syncTotalDataHelper] Provider更新後の状態: ${updatedState.totalLoginDays}日、${updatedState.totalWorkTimeMinutes}分',
  );

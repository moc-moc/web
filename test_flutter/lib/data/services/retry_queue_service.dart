...
class QueMk {
      ...
      if (index != -1) {
        ...
        switch (status) {
          ...
          case RetryStatus.failed:
            updatedItem = queue[index].markAsFailed(
  ...
  static Stream<bool> watchNetworkStatus({
  ...
  static Duration calculateRetryDelay(
    ...
    final exponentialDelay = Duration(
      seconds: baseDelay.inSeconds * (1 << retryCount),
    );
  ...
  static void startBackgroundProcessing({
    ...
    if (_isBackgroundProcessingActive) {
      LogMk.logInfo(
        'バックグラウンド処理は既に開始しています',
        tag: 'QueMk.startBackgroundProcessing',
      );
      return;
    }

    LogMk.logInfo(
      'バックグラウンド処理開始: ${interval.inSeconds}秒間隔',
      tag: 'QueMk.startBackgroundProcessing',
    );
  ...
  static void stopBackgroundProcessing() {
    if (!_isBackgroundProcessingActive) {
      LogMk.logInfo(
        'バックグラウンド処理は開始していません',
        tag: 'QueMk.stopBackgroundProcessing',
      );
      return;
    }

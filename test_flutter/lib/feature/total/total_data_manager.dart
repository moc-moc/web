...
class TotalDataManager {
  ...
  ///
  /// **戻り値**: 成功時true、失敗時false（キューに追加された場合もfalse）
  Future<bool> updateTotalDataWithRetry(
    String userId,
    TotalData totalData,
  ) async {
  ...
  ///
  /// **戻り値**: 成功時true、失敗時false（キューに追加された場合もfalse）
  Future<bool> saveTotalDataWithRetry(
    String userId,
    TotalData totalData,
  ) async {
  ...
  Future<Map<String, dynamic>> trackFinished({
    ...
    try {
      ...
      if (currentData == null) {
        ...
        debugPrint(
          '✅ [trackFinished] ローカル保存完了: totalLoginDays=${newData.totalLoginDays}, totalWorkTime=${newData.totalWorkTimeMinutes}分',
        );
        ...
        if (currentUser != null) {
          try {
            ...
            final firestoreSuccess = await _manager.saveWithRetry(
              userId,
              newData,
            );
      ...
      debugPrint(
        '✅ [trackFinished] ローカル更新完了: totalLoginDays=${updatedData.totalLoginDays}, totalWorkTime=${updatedData.totalWorkTimeMinutes}分',
      );
      ...
      return {
        'success': true,
        'message':
            'トラッキング完了！総ログイン: $newLoginDays日、総作業時間: ${formatWorkTime(newWorkTimeMinutes)}',
        'totalLoginDays': newLoginDays,
        'totalWorkTimeMinutes': newWorkTimeMinutes,
      };
    } catch (e) {

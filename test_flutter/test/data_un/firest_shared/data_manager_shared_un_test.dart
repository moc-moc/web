...
void main() {
  group('FirestoreDataManager', () {
    late FirestoreDataManager<TestItem> manager;

    setUp(() {
      manager = FirestoreDataManager<TestItem>(
    ...
    group('コンストラクタ', () {
      test('必須パラメータでFirestoreDataManagerを作成できる', () {
        final testManager = FirestoreDataManager<TestItem>(
        ...
        expect(testManager.storageKey, 'test_key');
      });

      test('カスタムフィールド名でFirestoreDataManagerを作成できる', () {
        final testManager = FirestoreDataManager<TestItem>(
          collectionPathBuilder: (userId) => 'users/$userId/test',
          fromFirestore: (data) => TestItem.fromJson(data),
          toFirestore: (item) => item.toJson(),
          storageKey: 'test_custom',
          toJson: (item) => item.toJson(),
          fromJson: (json) => TestItem.fromJson(json),
          idField: 'uid',
        ...
        expect(testManager.storageKey, 'test_custom');
      });
    });
    ...
    group('Phase 2: ローカル操作', () {
      late FirestoreDataManager<TestItem> localManager;

      setUp(() {
        localManager = FirestoreDataManager<TestItem>(
          collectionPathBuilder: (userId) => 'users/$userId/test_items',
          fromFirestore: (data) => TestItem(
            id: data['id'] as String,
            title: data['title'] as String,
      ...
      group('getLocalAll', () {
        test('ローカルから全アイテムを取得できる', () async {
          // 注意: 実際のSharedPreferences接続が必要なため、このテストは統合テストとして実行
          // 現在は基本的な構造のみをテスト
    ...
    group('Phase 3: 同期機能', () {
      late FirestoreDataManager<TestItem> syncManager;

      setUp(() {
        syncManager = FirestoreDataManager<TestItem>(
          collectionPathBuilder: (userId) => 'users/$userId/test_items',
          fromFirestore: (data) => TestItem(
            id: data['id'] as String,
            title: data['title'] as String,
      ...
      group('sync', () {
        test('FirestoreとSharedPreferencesを同期できる', () async {
          const testUserId = 'sync_test_user_123';
          
          // 注意: 実際のFirestoreとSharedPreferences接続が必要なため、このテストは統合テストとして実行
          // 現在は基本的な構造のみをテスト
    ...
    group('Phase 4-5: リトライ機能とキュー管理', () {
      late FirestoreDataManager<TestItem> retryManager;

      setUp(() {
        retryManager = FirestoreDataManager<TestItem>(
          collectionPathBuilder: (userId) => 'users/$userId/test_items',
          fromFirestore: (data) => TestItem(
            id: data['id'] as String,
            title: data['title'] as String,
    ...
    group('Phase 6: キュー統計と管理', () {
      late FirestoreDataManager<TestItem> queueManager;

      setUp(() {
        queueManager = FirestoreDataManager<TestItem>(
          collectionPathBuilder: (userId) => 'users/$userId/test_items',
          fromFirestore: (data) => TestItem(
            id: data['id'] as String,
            title: data['title'] as String,
